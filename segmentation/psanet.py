...
class PSA(nn.Module):
    def __init__(self, in_channels=2048, mid_channels=512, psa_type=2, compact=False, shrink_factor=2, mask_h=59,
        ...
        if psa_type == 2:
            ...
            self.attention_p = nn.Sequential(
                ...
                nn.BatchNorm2d(mid_channels),
                nn.ReLU(inplace=True),
                nn.Conv2d(mid_channels, mask_h*mask_w,
                          kernel_size=1, bias=False),
            )
    ...
    def forward(self, x):
        ...
        if self.psa_type in [0, 1]:
            ...
            if self.compact:
                if self.psa_type == 1:
                    y = y.view(n, h * w, h * w).transpose(1,
                                                          2).view(n, h * w, h, w)
            else:
                y = PF.psa_mask(y, self.psa_type, self.mask_h, self.mask_w)
            if self.psa_softmax:
                y = F.softmax(y, dim=1)
            x = torch.bmm(x.view(n, c, h * w), y.view(n, h * w, h * w)
            ...
            if self.shrink_factor != 1:
                h = (h - 1) // self.shrink_factor + 1
                w = (w - 1) // self.shrink_factor + 1
                x_col = F.interpolate(x_col, size=(
                    h, w), mode='bilinear', align_corners=True)
                x_dis = F.interpolate(x_dis, size=(
                    h, w), mode='bilinear', align_corners=True)
            y_col = self.attention(x_col)
            y_dis = self.attention_p(x_dis)
            if self.compact:
                y_dis = y_dis.view(
                    n, h * w, h * w).transpose(1, 2).view(n, h * w, h, w)
            else:
            ...
            x_col = torch.bmm(x_col.view(n, c, h * w), y_col.view(n, h * w, h * w)
                              ).view(n, c, h, w) * (1.0 / self.normalization_factor)
            x_dis = torch.bmm(x_dis.view(n, c, h * w), y_dis.view(n, h * w, h * w)
                              ).view(n, c, h, w) * (1.0 / self.normalization_factor)
...
if __name__ == '__main__':
    ...
    # crop_h = crop_w = 465
    crop_h, crop_w = 161, 241

    input = torch.rand(4, 3, crop_h, crop_w)
    compact = False
    ...
    if compact:
        ...
        assert (mask_h is None and mask_w is None) or (
            mask_h is not None and mask_w is not None)
        if mask_h is None and mask_w is None:
            mask_h = 2 * ((crop_h - 1) // (8 * shrink_factor) + 1) - 1
            mask_w = 2 * ((crop_w - 1) // (8 * shrink_factor) + 1) - 1
        else:
            assert (mask_h % 2 == 1) and (mask_h >= 3) and (
                mask_h <= 2 * ((crop_h - 1) // (8 * shrink_factor) + 1) - 1)
